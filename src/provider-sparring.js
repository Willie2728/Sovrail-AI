...
export function planProviderSparring({task={}, candidates=[], mode='on-demand', maxRuns=6, incumbent=null, force=false}={}) {
  ...
  const ordered = [...candidates].filter(c => c && c.provider && c.model).sort((a,b) => Number(a.estimatedCostUsd||0)-Number(b.estimatedCostUsd||0)).slice(0, Math.max(1, maxRuns));
  return {run:true, mode, fingerprint:fp, candidates:ordered, controls:{samePrompt:true,sameTaskInputs:true,recordActualLatency:true,recordActualCost:true,recordActualOutput:true}};
}

function inv(x, cap=1){ const n=Math.max(0, Number(x||0)); return 1/(1+Math.min(n,cap)); }
...
export function summarizeProviderSparring({task={},runs=[],weights={},minimumQuality=0.75}={}) {
  ...
  const report={fingerprint:fp,measured:true,taskClass:task.purpose||'unspecified',recommendation:best?{provider:best.provider,model:best.model,gear:best.gear||null,score:best.normalizedScore,reason:'Best measured successful-outcome score among tested candidates'}:null,runs:scored.sort((a,b)=>b.normalizedScore-a.normalizedScore),confidence:Number(confidence.toFixed(2)),generatedAt:new Date().toISOString()};
  taskProfiles.set(fp,{recommendation:report.recommendation,confidence:report.confidence,updatedAt:Date.now(),report});
  return report;
}

export function providerProfile(task={}) { return taskProfiles.get(taskClassFingerprint(task))||null; }
...
export function shouldAutoRespar({task={},drift={}}={}) {
  ...
  if(drift.modelChanged||drift.providerPricingChanged||drift.failureRateSpike||drift.qualityDrop||drift.latencySpike)return {run:true,reason:'Material provider/model drift detected'};
  if(Date.now()-profile.updatedAt>30*24*60*60*1000)return {run:true,reason:'Provider benchmark is older than 30 days'};
